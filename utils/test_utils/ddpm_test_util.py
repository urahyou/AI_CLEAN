...
class DDPMTestLoop(TestLoop):
    ...
    def forward_backward(self, data_item):
        global test_num
        img, batch_kwargs = data_item # data_item = {img, img_dir,}
    ...
    def save_samples(self, k_samples, samples_path, batch_kwargs):
        ...
        image = batch_kwargs["image"][0][0]       # 这个是GT
        image_dir = batch_kwargs["image_dir"][0][0]  # 这个是dirty image
        
        # 为了方便进行对比实验，这里把GT和dirty都保存成为numpy的形式
        np.save(os.path.join(samples_path, 'image.npy'), image)
        np.save(os.path.join(samples_path, 'image_dir.npy'), image_dir)
